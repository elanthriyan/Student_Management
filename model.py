...
CACHE_FILE = "db_faces.pkl"
attendance = set()
THRESHOLD = 0.75  # Face match threshold
...
# --- Helper Functions ---
def get_dataset_labels(dataset_path):
    """Return folder names (person names) in the dataset directory."""
    return sorted([d for d in os.listdir(dataset_path) if os.path.isdir(os.path.join(dataset_path, d))])

def process_reference_frames(img):
    """Extract embeddings from image using MTCNN + FaceNet."""
    face = mtcnn(img)
    embeddings = []
    if face is not None:
        if face.ndim == 4:
...
def load_or_update_cache():
    """Load existing cache if labels match dataset, else rebuild."""
    if os.path.exists(CACHE_FILE):
        with open(CACHE_FILE, 'rb') as f:
            data = pickle.load(f)
        cached_labels = sorted(list(set(data['labels'])))
        dataset_labels = get_dataset_labels(DB_PATH)

        if cached_labels == dataset_labels:
            print("✅ Dataset matches cache. Loading cached embeddings ...")
            return data['embeddings'], data['labels'], False
        else:
            print("🔁 Dataset changed! Rebuilding embeddings ...")
            return None, None, True
    else:
        print("⚠️ No cache found. Need to build embeddings.")
        return None, None, True

# --- Load or Rebuild Cache ---
db_embeddings, db_labels, rebuild = load_or_update_cache()

if rebuild:
    db_embeddings = []
    db_labels = []
    print("🔍 Processing dataset to extract embeddings ...")
